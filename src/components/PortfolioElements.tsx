...
function PortfolioElements() {
  ...
  const filterProjects =
    active === "All"
      ? projects
      : projects.filter((item) => item.cats.includes(active));
      ...
      <div className="flex gap-4 flex-wrap justify-center md:justify-start">
        {portCats.map((item) => (
          <Button
            ...
            className={`${
              active === item
                ? "bg-primary/50 hover:bg-primary/30 text-white"
                : "bg-white hover:bg-primary/30 hover:text-white"
            } rounded-full px-6 py-4 font-normal`}
        ...
        {filterProjects.map((project, idx) => (
          ...
          >
            ...
            <Link href={`/${project.id}`}>
              <h3 className="text-neutral-800 text-lg font-normal leading-[25px] mt-4 hover:underline">
                {project.title}
              </h3>
            </Link>
